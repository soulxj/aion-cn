...
public class Equipment {
		...
		if (item.getItemTemplate().isTwoHandWeapon()) {
			...
			if (itemInRightHand != null) {
				if (mainIsTwoHand) {
					...
					long switchSlot = itemInLeftHand == null ? leftSlot : rightSlot;
					if (rightSlot == switchSlot && (item.getEquipmentSlot() & ItemSlot.LEFT_HAND.getSlotIdMask()) != 0)
						switchSlot = leftSlot;
					if (validateOnly) {
						requiredInventorySlots++;
	...
	private boolean validateEquippedArmor(Item item, boolean validateOnly) {
		...
		if (itemInMainHand != null && armorType == ArmorType.SHIELD && itemInMainHand.getItemTemplate().isTwoHandWeapon()) {
			if (validateOnly) {
				...
				// remove 2H weapon
				unEquip(ItemSlot.MAIN_HAND.getSlotIdMask() | ItemSlot.SUB_HAND.getSlotIdMask());
			}
		}
		return true;
	}
	...
	public void onLoadHandler(Item item) {
		...
		if (template.isTwoHandWeapon()) {
			...
			ItemSlot[] allSlots = ItemSlot.getSlotsFor(template.getItemSlot());
			for (ItemSlot sl : allSlots) {
